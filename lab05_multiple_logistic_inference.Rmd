...
summary(fit1)
```
based on the result, it shows that claim_location is not important for prediction 
...
```
p-value is 0.2041. We fail to reject the null hypothesis. claim_location is important for prediction
...
```
p-value is 3.12*10^(-7), this shows that the variable "judge" is important for prediction.
...
If logit_success increases by one unit, the odds of the judge to grant an appeal will be e^1.3005=3.67 times higher.
